...
if (isset($_GET["id"]) && !empty($_GET["id"])) {
    ...
    $sql_check_related = "SELECT COUNT(*) FROM affectation WHERE Inventaire = ?";
    if ($stmt_check_related = $mysqli->prepare($sql_check_related)) {
        ...
        if ($related_count > 0) {
            ...
            $sql_delete_affectation = "DELETE FROM affectation WHERE Inventaire = ?";
    ...
    $sql_delete_materiel = "DELETE FROM materiel WHERE Inventaire = ?";
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger fade in">
                            <input type="hidden" name="Inventaire" value="<?php echo trim($_POST["Inventaire"]); ?>">
                            <p>Are you sure you want to delete this record?</p><br>
                            <p>
                                <input type="submit" value="Yes" class="btn btn-danger">
                                <a href="Lister_materiel.php" class="btn btn-default">No</a>
                            </p>
                        </div>
                    </form>
                </div>
